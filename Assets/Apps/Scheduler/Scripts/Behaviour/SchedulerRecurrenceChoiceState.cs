...
{
    public sealed class SchedulerRecurrenceChoiceState : AStateMachineBehaviour
    {
        ...
        override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
        {
            ...
            if (RecurrenceStatus.E_UI_DISPLAY == mRecurrenceStatus)
            {
                ...
                Buddy.Vocal.SayAndListen(Buddy.Resources.GetString(SchedulerDateManager.STR_RECC));
        ...
        private void OnEndListening(SpeechInput iSpeechInput)
        {
            ...
            if (!string.IsNullOrEmpty(iSpeechInput.Utterance)
                && SchedulerLanguageManager.GetInstance().GetRecurrenceLanguage().ExtractRecurrenceFromSpeech(
                                        ...
                                        ref lRepetitionDays))
            {
				...
				SchedulerDateManager.GetInstance().RepetitionTime = lRepetitionTime;
				SchedulerDateManager.GetInstance().RepetitionDays = lRepetitionDays;

                Buddy.GUI.Header.DisplayLightTitle(Buddy.Resources.GetString("eared") + Buddy.Resources.GetString(SchedulerDateManager.GetInstance().RepetitionTime.ToString().Replace("_", "").ToLower()));
        ...
        private void DisplayRecurrenceEntry()
        {
            ...
            Buddy.GUI.Toaster.Display<ParameterToast>().With((iOnBuild) =>
            {
                ...
                // Actions
                lOnce.OnClick.Add(() => { SchedulerDateManager.GetInstance().RepetitionTime = RepetitionTime.ONCE; });
                lDayly.OnClick.Add(() => { SchedulerDateManager.GetInstance().RepetitionTime = RepetitionTime.DAYLY; });
                lWeekly.OnClick.Add(() => { SchedulerDateManager.GetInstance().RepetitionTime = RepetitionTime.WEEKLY; });
                lTwoWeeks.OnClick.Add(() => { SchedulerDateManager.GetInstance().RepetitionTime = RepetitionTime.EVERY_TWO_WEEKS; });
                lMontly.OnClick.Add(() => { SchedulerDateManager.GetInstance().RepetitionTime = RepetitionTime.MONTHLY; });
                lAnnual.OnClick.Add(() => { SchedulerDateManager.GetInstance().RepetitionTime = RepetitionTime.ANNUAL; });
            },
