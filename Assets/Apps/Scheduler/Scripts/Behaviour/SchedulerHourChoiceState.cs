...
{
	public sealed class SchedulerHourChoiceState : AStateMachineBehaviour
	{
		...
		override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
		{
			...
			SchedulerDateManager.GetInstance().AppState = SchedulerDateManager.E_SCHEDULER_STATE.E_HOUR_CHOICE;
			...
			Buddy.Vocal.SayKeyAndListen(SchedulerDateManager.STR_WHOURS);
			...
			{
				SchedulerDateManager.GetInstance().SchedulerDate += new TimeSpan(DateTime.Now.Hour, DateTime.Now.Minute, 0);
			}
		}
		...
		private void OnEndListening(SpeechInput iSpeechInput)
		{
										...
										ref lDate))
			{
				...
				Buddy.GUI.Header.DisplayLightTitle(Buddy.Resources.GetString("eared") + SchedulerDateManager.GetInstance().SchedulerDate.ToShortTimeString());
		...
		private void DisplayHourEntry()
		{
			...
			Buddy.GUI.Toaster.Display<ParameterToast>().With((iOnBuild) =>
			{
				...
				mHourText.SetLabel(Buddy.Resources.GetString("hoursetto") + SchedulerLanguageManager.GetInstance().GetHourLanguage().HourToString(SchedulerDateManager.GetInstance().SchedulerDate));
		...
		private void GoToNextState(bool iManualValidation = false)
		{
			...
			DebugColor("HOUR IS: " + SchedulerDateManager.GetInstance().SchedulerDate.ToLongTimeString(), "green");
		...
		private void IncrementHour(int iIncrement)
		{
			...
			if (mHourModify == HourModify.HOUR)
				SchedulerDateManager.GetInstance().SchedulerDate = SchedulerDateManager.GetInstance().SchedulerDate.Add(new TimeSpan(iIncrement, 0, 0));
			else
				SchedulerDateManager.GetInstance().SchedulerDate = SchedulerDateManager.GetInstance().SchedulerDate.Add(new TimeSpan(0, iIncrement, 0));
			SchedulerDateManager.GetInstance().SchedulerDate = SchedulerDateManager.GetInstance().SchedulerDate.Subtract(new TimeSpan(0, 0, SchedulerDateManager.GetInstance().SchedulerDate.Second));
			mHourText.SetLabel(Buddy.Resources.GetString("hoursetto") + SchedulerLanguageManager.GetInstance().GetHourLanguage().HourToString(SchedulerDateManager.GetInstance().SchedulerDate));
		}
