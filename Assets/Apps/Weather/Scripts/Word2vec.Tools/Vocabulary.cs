    ...
    public class Vocabulary 
    {
        ...
        public Vocabulary(IEnumerable<Representation> representations, int vectorDimensionsCount)
        {
            ...
            foreach (var representation in representations)
            {
				...
				if (!(representation.WordOrNull == null || "".CompareTo(representation.WordOrNull) == 0 || " ".CompareTo(representation.WordOrNull) == 0) && !_dictioanary.ContainsKey(representation.WordOrNull))
        ...
        /// </summary>
		public Representation GetSummRepresentationOrNullForPhrase(string wordOrPhrase, bool withoutNull = false)
        {
			if (wordOrPhrase == null || "".CompareTo(wordOrPhrase) == 0 || " ".CompareTo(wordOrPhrase) == 0)
                return null;
			...
			return GetSummRepresentationOrNullForPhrase(words, withoutNull);
        }
        /// <summary>
        /// returns summ representation of target words. unknown words are ignored
        /// returns null if wordOrPhrase is empty or null or if all the words are unknown
        /// </summary>
		public Representation GetSummRepresentationOrNullForPhrase(string[] words, bool withoutNull = false)
        {
			if (withoutNull) {
				try {
					return words
						.Select (this.GetRepresentationFor)
						.Where (w => w != null)
						.SummOrNull ();
				} catch (Exception e) {
					return null;
				...
				return words 
					.Select (this.GetRepresentationOrNullFor)
					.Where (w => w != null)
					.SummOrNull ();
			}
        }
