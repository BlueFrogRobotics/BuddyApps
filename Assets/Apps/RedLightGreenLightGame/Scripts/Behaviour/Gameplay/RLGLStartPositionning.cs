        ...
        override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
        {
            ...
            if (!mHasTriggered)
            {
                ...
                if (mRLGLBehaviour.Timer > 0.1 && mDetectionCount <= 12 && mHasShowWindow)
                {
                    ...
                    {
                        mMat = mCam.FrameMat.clone();
                        mMatSrc = mCam.FrameMat.clone();
                        Core.flip(mMatSrc, mMat, 1);
                        Imgproc.rectangle(mMatSrc, new Point((int)(mMatSrc.width() / 3), 0), new Point((int)(mMatSrc.width() * 2 / 3), mMatSrc.height()), new Scalar(255, 255, 0), 3);
                        lTexture = Utils.MatToTexture2D(mMatSrc);
                        mTexture.SetPixels(lTexture.GetPixels());
                    }
                    else
                    {
                        lTexture = Utils.MatToTexture2D(mMatDetection);
                        mTexture.SetPixels(lTexture.GetPixels());
                        mMatDetection = null;
                    }
                    mTexture.Apply();
